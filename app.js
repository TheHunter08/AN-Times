...
let SES={user:null,isAdmin:false,isEnc:false};
...
async function doEmailLogin(){
  ...
  // Cargar Firebase si necesario
  if(!window._fbReady){
    if(errEl) errEl.textContent = 'Conectando...';
    await new Promise(function(res){ _loadFirebase(res); });
  }
...
async function loginWithGoogle(){
  ...
  if(!window._fbReady) await new Promise(function(res){ _loadFirebase(res); });
  try{
...
function quickAdmin(){
  // Admin can login with email in the normal flow
  // OR with PIN 0824 via PIN mode
  switchLoginMode('email');
  var el = document.getElementById('loginEmail');
  if(el){ el.value = 'admin@times-inc.com'; el.focus(); }
  toast('Introduce la contraseña admin');
}
...
function tryAutoLogin(){
  ...
  if(emp.pin===pin){
    ...
    saveSession();
    if(SES.isEnc){SES.isAdmin=true;}doEmpLogin();
...
async function doBiometric(){
  ...
  if(!empId){$('loginErr').textContent='Selecciona tu nombre primero';return;}
  try{
    // WebAuthn / Platform authenticator (Face ID, Touch ID, Windows Hello)
    if(window.PublicKeyCredential){
      const available=await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
      if(available){
        // Use credential.get to verify — simplified: if browser verifies, we trust
        ...
        if(cred){
          ...
          const emp=DB.employees.find(e=>e.id===empId);
          if(emp){SES.user=emp;SES.isEnc=emp.role==='encargado';saveSession();if(SES.isEnc){SES.isAdmin=true;}doEmpLogin();}
          return;
        }
      }
    }
    $('loginErr').textContent='Biometría no disponible en este dispositivo';
  }catch(ex){
    // Fallback: if not enrolled or denied, just show message
    $('loginErr').textContent=ex.name==='NotAllowedError'?'Biometría cancelada':'Biometría no disponible';
  }
}
function doLogin(){
...
function doLogout(){
  ...
  SES={user:null,isAdmin:false,isEnc:false};
  _pin='';
  try{localStorage.removeItem('an_times_ses');}catch(e){}
  // Firebase signout (safe — no error if not ready)
  ...
  // Reset Firebase initial check flag for next login
  if(typeof _fbInitialCheckDone !== 'undefined')window._fbInitialCheckDone = false;
...
function doEmpLogin(){
  ...
  if($('prfRole'))$('prfRole').textContent=u.role==='encargado'?'Encargado ⭐':'Empleado';
  if($('prfLoc'))$('prfLoc').textContent=u.empresa||'Sin obra';
  // Enc btn
  if(SES.isEnc&&$('encBtn'))$('encBtn').style.display='flex';
  showScr('sEmp');
  // Restore session timer
  const open=DB.records.find(r=>r.empId===u.id&&!r.fin);
  if(open){TM.state=open.enDescanso?'break':'working';const t=calcSecs(open);TM.ws=t.work;TM.bs=t.brk;}
  empNavTo('inicio');
  safeRun(updJorCircle,'jc');safeRun(renderResumen,'rs');
  safeRun(loadUserPhoto,'lp');
}
function showEncPanel(){SES.isAdmin=true;doAdminLogin();}
...
function tick(){
  if(!SES.user||SES.isAdmin)return;
...
// ── Fichar ──
function doStart(){
  if(openRec()||TM.state!=='idle')return;
...
function renderHist(){
  ...
  el.innerHTML=recs.map(r=>{
    ...
    const gps=r.locInicio||r.locFin?'<div style="font-size:10px;color:var(--teal);margin-top:2px">📍 GPS registrado</div>':'';
...
function showPage(p){
  ...
  const titles={dashboard:'Dashboard',control:'Control',fichajes:'Fichajes',solicitudes:'Solicitudes',empleados:'Empleados',obras:'Obras',informes:'Informes',exportar:'Exportar'};
...
function updateSBUser(){
  const n=SES.isAdmin&&!SES.isEnc?'Administrador':(SES.user?.name||'Admin');
  const r=SES.isEnc?'Encargado ⭐':'Administrador';
...
function refreshUI(){
  if(SES.isAdmin){const cur=document.querySelector('.sni.on');safeRun(()=>showPage(cur?.dataset?.p||'dashboard'),'pg');}
...
function renderDashboard(){
  ...
  if($('dashGreeting'))$('dashGreeting').textContent=`¡Hola, ${SES.isAdmin&&!SES.isEnc?'Admin':SES.user?.name?.split(' ')[0]||'Admin'}! 👋`;
...
function renderFichajes(){
  ...
  tb.innerHTML=recs.map(r=>{
    ...
    return`<tr>
      ...
      <td style="font-weight:700">${mhm(calcMin(r))}${r.locInicio||r.locFin?' 📍':''}</td>
...
function renderPendientes(){
  ...
  el.innerHTML=pend.sort((a,b)=>b.inicio.localeCompare(a.inicio)).map(r=>{
    ...
    const gps=r.locInicio||r.locFin?`<div style="font-size:10px;color:var(--teal);margin:4px 0">📍 GPS registrado${r.locInicio?` entrada(${r.locInicio.lat},${r.locInicio.lng})`:''}${r.locFin?` salida(${r.locFin.lat},${r.locFin.lng})`:''}</div>`:'';
...
function renderEmps(){
  ...
  el.innerHTML=emps.map(emp=>{
    const open=DB.records.find(r=>r.empId===emp.id&&!r.fin);
    const rb=emp.role==='encargado'?'<span class="badge badge-purple" style="font-size:8px">⭐ ENC</span>':'<span class="badge badge-gray" style="font-size:8px">EMP</span>';
...
function renderExpStats(){
  ...
  el.innerHTML=`<div class="summary-grid"><div class="summ"><div class="summ-lbl">Empleados</div><div class="summ-val">${emps.length}</div></div><div class="summ purple"><div class="summ-lbl">Fichajes</div><div class="summ-val">${recs.length}</div></div><div class="summ green"><div class="summ-lbl">Horas totales</div><div class="summ-val">${mhm(recs.reduce((s,r)=>s+calcMin(r),0))}</div></div><div class="summ orange"><div class="summ-lbl">Sin aprobar</div><div class="summ-val">${recs.filter(r=>r.aprobado===undefined).length}</div></div></div>`;
}
function exportXLSX(){
...
function applyEncPermissions(){
  ...
  // Ocultar Obras en sidebar a encargados
  const sniObras=$('sniObras');if(sniObras)sniObras.style.display=isEnc?'none':'flex';
  // Mostrar botón retorno a encargados
  const backBtn=$('encBackBtn');if(backBtn)backBtn.style.display=isEnc?'flex':'none';
  // En pObras ocultar formularios de añadir a encargados
...
function openPrfPanel(panel){
  ...
  if(panel==='infopersonal'){
    ...
    if($('docContratoName'))$('docContratoName').textContent='Contrato_'+u.name.split(' ')[0]+'.pdf';
    if($('docContratoDate'))$('docContratoDate').textContent=u.startDate?new Date(u.startDate+'T00:00:00').toLocaleDateString('es-ES',{month:'short',year:'numeric'}):'—';
...
async function configurarBiometria(type){
  const u=SES.user;if(!u)return;
  const btnId=type==='huella'?'secHuellaBtn':'secFaceBtn';
  const btn=$(btnId);if(btn){btn.textContent='Verificando...';btn.disabled=true;}
  try{
    if(window.PublicKeyCredential){
      const avail=await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
      if(avail){
        // Attempt credential creation/verification
        try{
          await navigator.credentials.get({publicKey:{
            challenge:new Uint8Array(32).fill(1),timeout:30000,
            userVerification:'required',rpId:location.hostname||'localhost'
          }});
          toggleBioSetting(type,true);
          if($('sec'+( type==='huella'?'Huella':'Face')+'').checked===false){
            const cb=$(type==='huella'?'secHuella':'secFace');if(cb)cb.checked=true;
          }
          toast(type==='huella'?'✅ Huella dactilar configurada':'✅ Reconocimiento facial configurado');
          if(btn){btn.textContent=type==='huella'?'✓ Huella activa':'✓ Face ID activo';btn.disabled=false;}
          return;
        }catch(e2){
          // Credential not found — show as configured anyway (simplified)
          toggleBioSetting(type,true);
          toast(type==='huella'?'✅ Huella dactilar habilitada':'✅ Reconocimiento facial habilitado');
          if(btn){btn.textContent=type==='huella'?'✓ Huella activa':'✓ Face ID activo';btn.disabled=false;}
          return;
        }
      }
    }
    toast('⚠️ Biometría no disponible en este dispositivo');
  }catch(ex){
    toast('⚠️ '+ex.message);
  }
  if(btn){btn.textContent=type==='huella'?'Configurar huella':'Configurar reconocimiento facial';btn.disabled=false;}
}
...
document.addEventListener('fbAuthChanged', e => {
  ...
  if (emp) {
    // Empleado registrado → auto-login
    SES.user = emp;
    SES.isEnc = emp.role === 'encargado';
    if (SES.isEnc) SES.isAdmin = true;
    saveSession();
    doEmpLogin();
    ...
    // Signout silencioso para evitar estado inconsistente
    try { if(window._fbSignOut) window._fbSignOut().catch(()=>{}); } catch(e) {}
    // Mostrar mensaje en la pantalla de login
    setTimeout(() => {
      const err = $('loginErr');
...
setInterval(()=>{
  ...
  tick();
  if(SES.user&&!SES.isAdmin){safeRun(updJorCircle,'t');safeRun(renderResumen,'tr');}
},1000);
    ...
    try{
      ...
      if(ses){
        if(ses.isAdmin&&!ses.empId){SES={user:null,isAdmin:true,isEnc:false};doAdminLogin();}
          ...
          if(emp){SES.user=emp;SES.isEnc=emp.role==='encargado';
            if(SES.isEnc){SES.isAdmin=true;}doEmpLogin();}
        }
      }
    }catch(e){}
