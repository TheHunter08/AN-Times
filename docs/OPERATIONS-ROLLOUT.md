...
- Paridad V1/V2: consistente, sin faltantes ni filas obsoletas; tres checkpoints consecutivos registrados.
- Backup posterior a la migración: verificado y restaurable (`backup-2026-08-11T17-59-34-517Z.json`).
...
- Versión transitoria actual: Times INC `4.5.1`, despliegue `dpl_Dy6qnTqZoPM2aBLWqnTtuziJHe2J`, publicado en `https://times-inc.vercel.app`. La integración compila con TypeScript limpio, 477 pruebas unitarias y 2 E2E Auth/RLS; el chequeo posterior mantiene 7/7 automatizaciones sanas y paridad tabular sin faltantes ni obsoletos.

Auditoría posterior a `dpl_DdKEucP9EMnagt9xBRDQ9Kh9F633`: producción responde `healthy`, las siete automatizaciones están sanas y la paridad V1/V2 mantiene cero faltantes y cero filas obsoletas. Continúan 12 perfiles activos, 2 identidades verificadas y 10 activaciones personales pendientes. No existen vínculos duplicados ni huérfanos. Hay al menos un `jefe_obra` vinculado, pero el perfil con rol `admin` todavía no ha completado su activación; el corte final sigue bloqueado para no perder el acceso administrativo principal.
