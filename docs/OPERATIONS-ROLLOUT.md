...
2. Crear o invitar los usuarios en Supabase Auth.
3. Vincular cada `employees.auth_id` con `auth.users.id`.
4. Confirmar que el resultado de readiness es `LISTO_PARA_PRUEBA_CONTROLADA`.
5. Hacer backup de `app_data` y las tablas V2.
6. Aplicar `supabase/policies_auth.sql` primero en staging.
7. Probar empleado, encargado, jefe de obra y administrador.
8. Activar en producción solo dentro de una ventana de mantenimiento con rollback preparado.
