...
const PendingCenter = lazy(() => import('./pages/PendingCenter.js').then(module => ({ default:module.PendingCenter })))
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.65)', zIndex: 1000, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: colors.bg[900], borderRadius: '16px 16px 0 0', border: `1px solid ${colors.border.default}`, padding: '24px 20px 40px', width: '100%', maxWidth: 480, maxHeight: '92dvh', overflowY: 'auto', boxShadow: '0 -24px 64px rgba(0,0,0,.6)', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <button onClick={handleSave} disabled={sending} style={{ padding: '12px', borderRadius: 10, border: 'none', background: colors.primary.base, color: '#fff', fontSize: 14, fontWeight: 700, cursor: sending ? 'wait' : 'pointer', fontFamily: 'inherit', marginTop: 4, opacity: sending ? 0.7 : 1 }}>
          {sending ? 'Guardando…' : isEdit ? 'Guardar cambios' : 'Crear empleado'}
        </button>
      </div>
    </div>
  )
  ...
  return (
    <Dashboard
      ...
      quickLinks={[
        ...
        { id: 'fichajes',    label: 'Trabajando ahora',  value: data.kpis[1]?.value || '0', onClick: () => onNavigate('en_linea') },
  ...
  const { weekLabel, employees } = useMemo(() => {
    ...
    const employees = emps.map((e: any) => ({
      ...
      week: weekDays.map(d => {
        ...
        // Un turno explícito (guardia, jornada extraordinaria, etc.) también
        // debe mostrarse en sábado o domingo. Antes el corte por fin de semana
        // ocurría primero y ocultaba asignaciones reales.
        if (!turno) return {}
        return {
...
export default function AppV2Admin() {
  ...
  function renderPage() {
    ...
    if (page === 'pendientes')     return <PendingCenter onNavigate={setAdminPage} />
