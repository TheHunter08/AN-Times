...
export function Table<T>({ columns, rows, rowKey, emptyLabel = 'Sin resultados' }: TableProps<T>) {
  if (!rows.length) {
    return <div style={{ padding: 32, textAlign: 'center', color: colors.text[500], fontSize: 13 }}>{emptyLabel}</div>
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      {rows.map(row => (
        ...
        >
          {columns.map(col => (
            <div key={col.key} style={{ flex: col.width ? `0 0 ${col.width}` : 1, fontSize: 13, color: colors.text[900] }}>
              {col.render(row)}
            </div>
          ))}
        </div>
