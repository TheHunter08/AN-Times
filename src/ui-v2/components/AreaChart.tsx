...
export function AreaChart({ data, height = 180, color = colors.primary.base }: AreaChartProps) {
  const gradId = useId()
  const w = 100, h = 100
  const n = data.length
  const stepX = n > 1 ? w / (n - 1) : w
  const points = data.map((d, i) => [i * stepX, h - (Math.max(0, Math.min(100, d.value)) / 100) * h] as const)
