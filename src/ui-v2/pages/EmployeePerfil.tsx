// Página "Perfil" — versión ui-v2. Misma lógica que TabPerfil.jsx (legacy),
// relocalizada sin cambios de negocio.
...
function MenuRow({ icon, iconBg, label, sub, badge, badgeColor, onClick }: { icon: ReactNode; iconBg: string; label: string; sub?: string; badge?: string | number; badgeColor?: string; onClick: () => void }) {
  return (
    ...
    >
      <div style={{ width: 34, height: 34, borderRadius: radius.sm, background: iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
        {icon}
      </div>
  ...
  return (
    <div style={{ margin: '0 16px' }}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 8, paddingLeft: 2 }}>
        <div style={{ fontSize: 'var(--font-caption)', fontWeight: 'var(--font-semibold)', color: colors.text[500] }}>{title}</div>
      </div>
      <div style={{ background: 'var(--gradient-card), var(--bg-card)', border: `1px solid ${colors.border.subtle}`, borderRadius: radius.xl, overflow: 'hidden' }}>
        {children}
      </div>
    </div>
  )
...
export function EmployeePerfil({ u, db, saveDB, toast, doLogout, openModal, perfilView = 'perfil', setPerfilView }: EmployeePerfilProps) {
  ...
  if (!db.records) return (
    <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
      {[140, 80, 200].map((h, i) => <div key={i} className="skeleton" style={{ height: h, borderRadius: 14 }} />)}
    </div>
  )

  const initials = u.initials || u.name.split(' ').filter(Boolean).map((n: string) => n[0]).join('').slice(0, 2).toUpperCase() || '?'
  ...
  const yearRecs = myRecs.filter((r: any) => r.inicio && localDateStr(new Date(r.inicio)).startsWith(yearStr))
    ...
    <PullToRefresh>
      <div className="employee-profile-v2" style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', maxWidth: 520, margin: '0 auto', paddingBottom: 'calc(100px + env(safe-area-inset-bottom))' }}>
        ...
        <div style={{
          ...
          <div style={{
            width: 72, height: 72, borderRadius: radius.xl, background: avatarColor,
        ...
        <div style={{ padding: '0 16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[
            {
              val: mhm(monthMin), lbl: 'Horas este mes', accent: colors.primary.base, color: colors.primary.light,
              icon: (c: string) => <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="9" /><polyline points="12 7 12 12 15 14" /></svg>,
            },
            {
              val: String(vac.available), lbl: 'Vacaciones disp.', accent: colors.semantic.green, color: colors.semantic.green,
              icon: (c: string) => <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg>,
            },
            {
              val: `${streak}`, lbl: 'Racha actual', accent: colors.semantic.orange, color: colors.semantic.orange,
              icon: (c: string) => <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round"><path d="M12 2c0 6-6 8-6 14a6 6 0 0 0 12 0c0-6-6-8-6-14z" /><path d="M12 12c0 3-2 4-2 6a2 2 0 0 0 4 0c0-2-2-3-2-6z" /></svg>,
            },
            {
              val: yearDays > 0 ? `${yearDays}` : '—', lbl: `Días ${now.getFullYear()}`, accent: colors.primary.base, color: colors.primary.light,
              icon: (c: string) => <svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round"><rect x="3" y="4" width="18" height="18" rx="2" /><line x1="16" y1="2" x2="16" y2="6" /><line x1="8" y1="2" x2="8" y2="6" /><line x1="3" y1="10" x2="21" y2="10" /></svg>,
            },
          ].map(({ val, lbl, accent, color, icon }) => (
            <div key={lbl} style={{ background: colors.bg[600], border: `1px solid ${colors.border.subtle}`, borderRadius: radius.xl, padding: '16px 14px', position: 'relative', overflow: 'hidden' }}>
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 3, background: accent, borderRadius: '12px 12px 0 0' }} />
              <div style={{ width: 34, height: 34, borderRadius: radius.md, background: toneSoft(accent, 13), display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 12 }}>{icon(color)}</div>
        ...
        {pendingCierres.length > 0 && (
          <button type="button" onClick={() => openModal('cierreSign')} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: colors.text[500], marginTop: 2 }}>
                {pendingCierres.map((c: any) => c.mes).join(', ')} · Toca para revisar y firmar
              </div>
            </div>
            <span style={{ minWidth: 22, height: 22, borderRadius: radius.pill, background: colors.semantic.orange, color: '#fff', fontSize: 11, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 5px', flexShrink: 0 }}>{pendingCierres.length}</span>
          </button>
        )}
        ...
        <SectionCard title="Más opciones">
          <MenuRow
            ...
            iconBg="var(--success-soft)" label="Gastos" onClick={() => setPerfilView('gastos')}
          />
          <RowDivider />
          <MenuRow
            ...
            iconBg={toneSoft(colors.primary.base, 13)} label="Denuncia" onClick={() => setPerfilView('denuncia')}
          />
        </SectionCard>
        ...
        <SectionCard title="Mi cuenta">
          <MenuRow
            ...
            iconBg={toneSoft(colors.primary.base, 13)} label="Información personal" onClick={() => openModal('infoPersonal')}
          />
          <RowDivider />
          <MenuRow
            ...
            iconBg="var(--info-soft)" label="Documentos" sub={pendingDocs > 0 ? `${pendingDocs} pendiente${pendingDocs > 1 ? 's' : ''} de firma` : undefined}
            badge={pendingDocs > 0 ? pendingDocs : undefined} onClick={() => openModal('documentos')}
          />
          <RowDivider />
          <MenuRow
            ...
            iconBg={toneSoft('var(--accent-400)', 13)} label="Firma digital" sub={!hasFirma ? 'Sin configurar' : undefined}
            badge={!hasFirma ? '!' : undefined} onClick={() => openModal('sign')}
          />
          <RowDivider />
          <MenuRow
            ...
            iconBg="var(--success-soft)" label="Mi código QR" onClick={() => openModal('miQR')}
          />
        </SectionCard>
        ...
        <SectionCard title="Personalización">
          <MenuRow
            ...
            iconBg={colors.secondary.dim} label="Configuración" onClick={() => openModal('configuracion')}
          />
          <RowDivider />
          <MenuRow
        ...
        <div style={{ padding: '0 16px' }}>
          ...
          >
            <div style={{ width: 34, height: 34, borderRadius: radius.sm, background: 'var(--danger-soft)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
