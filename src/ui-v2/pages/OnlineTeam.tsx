...
export function OnlineTeam({ rows, hasScope }: { rows: OnlineTeamRow[]; hasScope: boolean }) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 300px), 1fr))', gap: 12 }}>
          {rows.map(row => (
            <Card key={row.id} padding={4} style={{ minHeight: 128 }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, marginTop: 18, paddingTop: 12, borderTop: `1px solid ${colors.border.subtle}`, fontSize: 12 }}>
