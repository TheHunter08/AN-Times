...
export function ModalTemas({ visible, db, u, onClose, saveDB }) {
  ...
  return (
    <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="themes-dialog-title" style={{ position:'fixed', inset:0, zIndex:200, display:'flex', flexDirection:'column', background:colors.bg[800] }}>
      <div style={{ display:'flex', alignItems:'center', gap:12, padding:'16px 16px 12px', borderBottom:`1px solid ${colors.border.default}`, flexShrink:0 }}>
        <button onClick={onClose} aria-label="Cerrar temas" style={{ background:'none', border:'none', color:colors.text[500], cursor:'pointer', padding:4, display:'flex' }}>
        ...
        <div id="themes-dialog-title" style={{ fontSize:16, fontWeight:800, color:colors.text[900] }}>Temas</div>
      </div>
