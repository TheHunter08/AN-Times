...
// Pasos: Correo(0) → Notificaciones(1) → Firma(2) → Recordatorio(3)
const STEP_EMAIL = 0
const STEP_NOTIFICATIONS = 1
const STEP_SIGNATURE = 2
const STEP_REMINDER = 3
...
export function OnboardingModal({ visible, u, db, saveDB, toast, pushReady, notificationPermission, onActivateNotifications, hasOpenShift, onFinishOpenShift }) {
  ...
  const existingEmail = (db.employees || []).find(e => e.id === u?.id)?.email || u?.email || ''
  const emailReady = isValidAccountEmail(existingEmail)
  const isRecheck = Boolean(existingSignature && emailReady)

  const [step, setStep] = useState(() => (!emailReady ? STEP_EMAIL : STEP_NOTIFICATIONS))
  ...
  const saveEmailAndContinue = () => {
    if (emailReady) { setStep(STEP_NOTIFICATIONS); return }
  ...
  const STEPS = ['Correo', 'Notificaciones', 'Tu firma', 'Recordatorio']
