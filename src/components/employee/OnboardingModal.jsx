...
// Pasos: Notificaciones(0) → Firma(1) → Recordatorio(2)
// Correo y vinculación de cuenta ya NO viven aquí: LoginV2.tsx intercepta el
// PIN antes de completar el login (handlePinKey) y exige activar la cuenta
// oficial (correo + contraseña, PIN verificado en servidor vía
// /api/activate-account) antes de llamar a doLogin(). Por construcción, quien
// llega hasta este modal ya tiene email y cuenta vinculados — mantener un
// segundo paso aquí duplicaba esa comprobación con una versión más débil
// (verificación de PIN en cliente) y confundía a los empleados con dos
// pantallas de "activa tu cuenta" distintas.
const STEP_NOTIFICATIONS = 0
const STEP_SIGNATURE = 1
const STEP_REMINDER = 2
...
export function OnboardingModal({ visible, u, db, saveDB, toast, pushReady, notificationPermission, onActivateNotifications, hasOpenShift, onFinishOpenShift }) {
  ...
  const [step, setStep] = useState(STEP_NOTIFICATIONS)
