...
const colors = {
  bg: { 400: 'var(--bg-card-hover)', 500: 'var(--bg-elevated)', 600: 'var(--bg-card)', 700: 'var(--bg-canvas-soft)', 800: 'var(--bg-canvas)' },
  primary: {
  ...
  kpiTone: { amber: { base: 'var(--warning-400)' } },
}
...
export function TabJornada({ timer, db, u, toast, saveDB, openModal, closeModal, activeModal, modalData }) {
  ...
  const recs = (db.records || []).filter(r => r.empId === u.id && r.inicio && localDateStr(new Date(r.inicio)) === todayStr).sort((a,b) => a.inicio.localeCompare(b.inicio))
    ...
    <PullToRefresh>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)', maxWidth: 520, margin: '0 auto', padding: 'var(--space-4)', paddingBottom: 'calc(100px + env(safe-area-inset-bottom))' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          <button type="button" aria-expanded={showRangeExport} onClick={() => setShowRangeExport(v => !v)} style={{
        ...
        {showRangeExport && (
          <div style={{
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button type="button" onClick={() => setShowRangeExport(false)} style={{
              ...
              <button type="button" onClick={exportRangePDF} disabled={generatingRangePdf} style={{
      ...
      return (
        <div role="dialog" aria-modal="true" aria-label="Vista previa del registro de jornada" style={{ position: 'fixed', inset: 0, zIndex: 300, minHeight: '100dvh', background: colors.bg[800], display: 'flex', flexDirection: 'column', paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
          <div style={{
            ...
            <button type="button" onClick={closeInforme} style={{
